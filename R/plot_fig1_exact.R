plot_fig1_exact <- function(dat_plot,y_min,y_max){
...
plot + theme_bw() +
  ...
  theme(panel.grid = element_blank(),
        plot.caption = element_text(hjust = 0,
                                    size = 14)) +
  scale_color_manual(name = "Treatment group", values = c("blue", "red")) +
  scale_linetype_manual(name = "Treatment group", values = c("solid", "dashed"))+
  scale_shape_manual(name = "Treatment group", values = c(19, 18)) +
  scale_y_continuous(
    name = "HbA1c (%)",
